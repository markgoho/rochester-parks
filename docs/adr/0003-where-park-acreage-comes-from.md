...
- The front matter can now record the source of a figure, in `acresSource`: `official page`, `park database` or `city GIS layer`, matching the order above. It is optional; none of the 227 Parks that had `acres` before this decision carry it, and nothing backfills them.
- Every process that writes `acres` also writes `acresSource`. A Park with no `acresSource` gets one the next time a sync or crawl writes its `acres`.
- A sync must read `acresSource` before it writes `acres`. It must not overwrite a figure whose `acresSource` is `official page` with a `park database` or `city GIS layer` figure. A Park with no `acresSource` is treated as before: the sync reads the official page itself before writing.
